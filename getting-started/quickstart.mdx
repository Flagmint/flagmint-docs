...
<Warning>
  **Always wrap `ready()` in a try/catch.** If the Flagmint API is unreachable, `ready()` will reject. Cached flags may still be available via `getFlags()` — see the [Error Handling](/sdk/nodejs#error-handling) section for details.
</Warning>
...
<CardGroup cols={2}>
  <Card title="SDK Reference" icon="js" href="/sdk/nodejs">
